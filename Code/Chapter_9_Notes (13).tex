...
\draw[blue,domain=-1.5:1.5,samples=100] plot (\x,{exp(\x)});
\draw[red,domain=-1.5:1.5] plot (\x,{1+\x});
\draw[green,domain=-1.5:1.5] plot (\x,{1+\x+0.5*\x^2});
\draw[magenta,domain=-1.5:1.5] plot (\x,{1+\x+0.5*\x^2+1/6*\x^3});
\draw[dashed] (1,-0.2) -- (1,2.7);
\node[below] at (1,-0.2) {$1$};
\node[left] at (-0.2,1) {$1$};
\node[blue,right] at (1.5,{exp(1.5)}) {$e^x$};
\node[red] at (0.5,1) {$P_1(x)$};
\node[green] at (1.7,2.8) {$P_2(x)$};
\node[magenta] at (1.9,3.8) {$P_3(x)$};
\end{tikzpicture}
\end{center}
